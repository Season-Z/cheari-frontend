import { userTypeEnum } from '@/common/user';
    ...
    >
      <div className={`cursor-pointer radius-8 px-[8px] py-[2px] transition ease-in-out duration-500 ${focusBg}`}>
        <div className="flex justify-start items-center gap-3">
          <Avatar size={32} src={`http://localhost:3280/${user?.userAvatar}`} />
          <div className="w-[110px] text-text-8">
            <div className="w-full title-14 truncate">{user?.userUsername}</div>
            <div className="w-full text-12 truncate">{userTypeEnum[user?.userType]}</div>
          </div>
          <Icon
